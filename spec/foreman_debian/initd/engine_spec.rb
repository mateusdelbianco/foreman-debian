...
describe ForemanDebian::Initd::Engine, :fakefs do
  ...
  it 'creates script' do
    script = engine.create_script('foo', 'foo arg1 arg2', 'app-user', 'QUIT', '/dir_root')
    ...
    expect(script.dir_root).to be == '/dir_root'
  end

  it 'installs script' do
    script = engine.create_script('foo', 'bar arg1 arg2', 'app-user', 'TERM', '/app')
  ...
  it 'installs script with custom quit' do
    script = engine.create_script('foo', 'bar arg1 arg2', 'app-user', 'QUIT', '/custom/path')
    engine.install(script)
